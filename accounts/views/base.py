...
class Base(APIView):
    def get_enterprise_user(self, user_id):
        ...
        groups: BaseManager[User_Groups] = User_Groups.objects.filter(
            user_id=user_id
        ).all()

        for g in groups:
            group: Group = g.group
            permissions: BaseManager[Group_Permissions] = (
                Group_Permissions.objects.filter(group_id=group.id).all()
            )
