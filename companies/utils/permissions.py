...
def check_permission(user, method, permission_to: str) -> bool:
    ...
    groups: ValuesQuerySet[User_Groups, dict[str, Any]] = User_Groups.objects.values('group_id').filter(user_id=user.id).all()
    
    for group in groups:
        permissions: ValuesQuerySet[Group_Permissions, dict[str, Any]] = Group_Permissions.objects.values('permission_id').filter(group_id=group['group_id']).all()
...
class EmployeePermission(permissions.BasePermission):
    ...
    def has_permission(self, request, _view) -> bool: # type: ignore     
        return check_permission(user=request.user, method=request.method, permission_to='employee')
    
class GroupPermission(permissions.BasePermission):
    message= 'O funcionario não tem permissão para gerenciar os grupos'
    
    def has_permission(self, request, _view) -> bool: # type: ignore
        return check_permission(user=request.user, method=request.method, permission_to='group')
...
class GroupsPermissionsPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, _view) -> bool: # type: ignore
        return check_permission(user=request.user, method=request.method, permission_to='permission')
    
class TaskPermission(permissions.BasePermission):
    message= 'O funcionario não tem permissão para gerenciar as tarefas de todos os funcionários'
    
    def has_permission(self, request, _view) -> bool: # type: ignore
        return check_permission(user=request.user, method=request.method, permission_to='task')
